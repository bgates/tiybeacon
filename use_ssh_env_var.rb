require 'tempfile'
file = Tempfile.new('pem')
begin
  puts 'writing ssh private key file...'
  file.write(ENV['RETHINKDB_SSH'])
  puts 'file was written'
  Dir.mkdir("./.ssh")
  puts 'storing known_hosts...'
  File.open("./.ssh/known_hosts", "w") do |f|
    f.puts "ec2-52-23-215-146.compute-1.amazonaws.com,52.23.215.146 ssh-rsa "
  end
  puts 'about to run following:'
  puts "ssh ubuntu@#{ENV['AWS_DNS']} -vvv -f -i #{file.path} -o StrictHostKeyChecking=no -p 2222 -N -L 28015:#{ENV['AWS_IP']}:28015"
  `ssh -vvv -f -i #{file.path} -o StrictHostKeyChecking=no -N ubuntu@#{ENV['AWS_DNS']}  -p 2222 -L 28015:#{ENV['AWS_IP']}:28015`
  puts 'please'
ensure
  file.close
  file.unlink
end
